// src/components/CyberAwareness.jsx
import React, { useState, useMemo } from 'react';
import { Search, Filter, Play, Eye, Clock, User, X } from 'lucide-react';
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12 space-y-6">
          ...
          <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Showing <span className="text-purple-400 font-semibold">{filteredVideos.length}</span> of <span className="text-purple-400 font-semibold">{videosData.length}</span> videos
          </div>
        </div>
      ...
      <div className={`rounded-lg overflow-hidden transition-all duration-300 border ${
        ...
        <div className="p-4">
          ...
          <div className={`flex items-center justify-between text-xs ${isDark ? 'text-gray-500' : 'text-gray-600'}`}>
            ...
            <span className={isDark ? 'text-gray-600' : 'text-gray-500'}>{video.uploadDate}</span>
          </div>
        </div>
      </div>
    </div>
